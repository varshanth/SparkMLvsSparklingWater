...
for algo in ['lr', 'kmeans','pca', 'mlp']:
    ...
    for chunk in chunks:
        ...
        try:
            print( "Running cmd", pyspark_run_cmd )
            subprocess.run(pyspark_run_cmd, shell=True, check=True, env=prod_env)
        except:
        ...
        try:
            print( "Running cmd", pysparkling_run_cmd )
            subprocess.run(pysparkling_run_cmd, shell=True, check=True, env=prod_env)
        except:
